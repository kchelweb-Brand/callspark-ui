...
import { findContactByPhone, normalizeNumber } from "./phone-numbers";
...
const FALLBACK_MENU_GREETING = "Thank you for calling. Please listen to the following options.";
const FALLBACK_VOICEMAIL_GREETING =
...
async function findTenantByNumber(to: string): Promise<{ tenantId: string } | null> {
  if (!to) return null;
...
async function menuIdForNumber(tenantId: string, to: string): Promise<string | null> {
  const normalized = normalizeNumber(to);
  const rows = await sql`
...
async function loadConfig(tenantId: string): Promise<TenantPhoneConfig> {
  const rows = await sql`
...
function findMenu(config: TenantPhoneConfig, id: string | null | undefined): IvrMenu | null {
  if (!id) return null;
 ...
 */
function targetsForExtension(config: TenantPhoneConfig, reference: string): string[] {
...
function ringTargetsFromRouting(config: TenantPhoneConfig): string[] {
 ...
 */
function dialableTarget(value: string): string | null {
